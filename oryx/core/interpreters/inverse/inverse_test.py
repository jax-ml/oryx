...
class InverseTest(test_util.TestCase):
  ...
  def test_advanced_inverse_two(self):
    ...
    np.testing.assert_allclose(f_inv(2.0, 2.0),
                                (jnp.log(2.), 2 - jnp.log(2.) ** 2))

  def test_advanced_inverse_three(self):
    def f(x, y, z):
      return jnp.exp(x), x ** 2 + y, jnp.exp(z + y)
    f_inv = core.inverse(f, 0., 0., 0.)
    np.testing.assert_allclose(f_inv(2.0, 2.0, 2.0),
                                (jnp.log(2.), 2 - jnp.log(2.) ** 2,
                                 jnp.log(2.0) - (2 - jnp.log(2.) ** 2)))
  ...
  def test_mul_inverse_ildj(self):
    ...
    np.testing.assert_allclose(ildj_, -jnp.log(jnp.abs(jax.jacrev(f)(1.))),
                                atol=1e-6, rtol=1e-6)
  ...
  def test_lower_triangular_jacobian(self):
    ...
    np.testing.assert_allclose(ildj_, -jnp.log(
  ...
  def test_div_inverse_ildj(self):
    ...
    np.testing.assert_allclose(ildj_, -jnp.log(jnp.abs(jax.jacrev(f)(4.))),
                                atol=1e-6, rtol=1e-6)
    ...
    np.testing.assert_allclose(ildj_, -jnp.log(jnp.abs(jax.jacrev(f2)(1.5))),
                                atol=1e-6, rtol=1e-6)
  ...
  def test_inverse_of_pmap(self):
    ...
    np.testing.assert_allclose(ildj_,
                                -jnp.log(jnp.abs(jnp.sum(jax.jacrev(f)(
                                    jnp.log(2.) * x)))),
                                atol=1e-6, rtol=1e-6)
  ...
  def test_sigmoid_ildj(self):
    ...
    with self.assertRaises(AssertionError):
      np.testing.assert_allclose(naive_inv(0.9999),
                                  jax.scipy.special.logit(0.9999))
    with self.assertRaises(AssertionError):
      np.testing.assert_allclose(naive_ildj(0.9999),
    ...
    np.testing.assert_allclose(f_ildj(0.9999),
                                tfb.Sigmoid().inverse_log_det_jacobian(
                                    0.9999, 0))
  ...
  def test_logit_ildj(self):
    ...
    np.testing.assert_allclose(f_ildj(-100.),
