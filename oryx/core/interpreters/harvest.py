...
def sow_cond(
  ...
  with jax_core.take_current_trace() as trace:
    return _sow(trace, value, tag=tag, name=name, mode=mode, key=key, pred=pred)[0]
...
class ReapContext(HarvestContext):
  ...
  def process_custom_jvp_call(self, trace, primitive, fun, jvp, vals_in, *,
                              symbolic_zeros):
    ...
    out_flat = primitive.bind_with_trace(
      trace.parent_trace, (fun, jvp, *vals_in),
  ...
  def process_custom_vjp_call(self, trace, primitive, fun, fwd, bwd, vals_in,
                              out_trees, symbolic_zeros):
    fun, aux1 = reap_eval(fun, self.settings)
    ...
    out_flat = primitive.bind_with_trace(
      trace.parent_trace, (fun, fwd, bwd, *vals_in),
...
def _reap_scan_rule(trace: HarvestTrace, *vals, length, reverse, jaxpr,
  ...
  _, carry_avals, xs_avals = tree_util.tree_map(
      lambda x: jax_core.get_aval(x), (const_vals, carry_vals, xs_vals))
  ...
  out = lax.scan_p.bind_with_trace(trace.parent_trace,
...
def _reap_while_rule(trace: HarvestTrace, *tracers, cond_jaxpr, body_jaxpr,
  ...
  _, init_avals = tree_util.tree_map(lambda x: jax_core.get_aval(x),
  ...
  out = lax.while_p.bind_with_trace(trace.parent_trace,
...
def _reap_cond_rule(trace, *tracers, branches, linear=None):
  ...
  _, ops_avals = tree_util.tree_map(lambda x: jax_core.get_aval(x),
                                    (index_val, ops_vals))
  settings = trace.context.settings
  reap_settings = dict(
      tag=settings.tag,
  ...
  if linear is None:
    out = lax.cond_p.bind_with_trace(trace.parent_trace,
...
def _reap_pjit_rule(trace, *invals, **params):
  ...
  outvals = pjit.pjit_p.bind_with_trace(trace.parent_trace, (*final_consts, *invals), new_params)
...
def _plant_scan_rule(trace: HarvestTrace, *tracers, length, reverse, jaxpr,
  ...
  carry_avals, xs_avals = tree_util.tree_map(lambda x: jax_core.get_aval(x),
                                             (carry_vals, xs_vals))
  settings = trace.context.settings
  ...
  out = lcf.scan_p.bind_with_trace(trace.parent_trace,
...
def _plant_while_rule(trace: HarvestTrace, *tracers, cond_jaxpr, body_jaxpr,
  ...
  cond_const_vals, body_const_vals, init_vals = jax_util.split_list(
      tracers, [cond_nconsts, body_nconsts])
  init_avals = tree_util.tree_map(lambda x: jax_core.get_aval(x), init_vals)
  settings = trace.context.settings
  body_metadata = _get_harvest_metadata(body_jaxpr, settings,
                                        *(body_const_vals + init_vals))
  for k, meta in body_metadata.items():
  ...
  out = lcf.while_p.bind_with_trace(trace.parent_trace,
...
def _plant_cond_rule(trace, *tracers, branches, linear=None):
  ...
  index_val, ops_vals = tracers[0], tracers[1:]
  ops_avals = tree_util.tree_map(lambda x: jax_core.get_aval(x), ops_vals)
  settings = trace.context.settings
  plant_settings = dict(
      tag=settings.tag,
      allowlist=settings.allowlist,
